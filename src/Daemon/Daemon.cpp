  ...
  command_line::add_arg(desc_cmd_sett, arg_genesis_block_reward_address);
    ...
    bool r = command_line::handle_error_helper(desc_options, [&]()
    {
      ...
      else
      {
        ...
        std::cout << "Windows:   Bitcoinnovad.exe --config-file configs/dashcoin.conf" << std::endl;
        std::cout << "Linux/Mac:   ./Bitcoinnovad --config-file configs/dashcoin.conf" << std::endl;
        return false;
      }
      po::notify(vm);
